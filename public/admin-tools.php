...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        ...
        switch ($action) {
            ...
            case 'fix_cache_path':
                $output = "Fix Cache Path (\"Please provide a valid cache path.\"):\n\n";
                ...
                foreach ($pathsToEnsure as $path) {
                    ...
                    $output .= ($writable ? '✅' : '❌') . " Writable: {$path} (Perms: {$perms})\n";
                }
            ...
            case 'seed_users_only':
                $output = "Seeding Users Only:\n";
                $seeders = [
                    'Database\\\\Seeders\\\\SuperAdminSeeder',
                    'Database\\\\Seeders\\\\AdminSeeder',
                    'Database\\\\Seeders\\\\MarketingSeeder',
                    'Database\\\\Seeders\\\\CSSeeder',
                    'Database\\\\Seeders\\\\BrandOwnerSeeder',
                ];

                foreach ($seeders as $seeder) {
                    $output .= "\n--- {$seeder} ---\n";
                    $output .= executeCommand("php artisan db:seed --class={$seeder} --force");
                    $output .= "\n";
                }
                break;
            ...
            <div class="grid">
                <div class="card">
                    ...
                    <form method="post" style="margin: 0;">
                        ...
                        <button type="submit" name="action" value="fix_cache_path" class="warning">Fix "valid cache path"</button>
                    </form>
                </div>
                ...
                <div class="card">
                    ...
                    <form method="post" style="margin: 0;">
                        ...
                        <button type="submit" name="action" value="seed_users_only" class="success">Seed Users Only</button>
