...
const form = useForm({
  ...
  transaksi: props.item.transaksi || 0,
  keterangan: props.item.keterangan || '',
})

const transaksiFormatted = ref(new Intl.NumberFormat('id-ID').format(form.transaksi))
...
const submit = () => {
  form.put(`/cs/repeats/${props.item.id}`)
}

const breadcrumbs = [
  { title: 'Dashboard', href: '/dashboard' },
  { title: 'CS Repeat', href: '/cs/repeats' },
  { title: 'Edit', href: `/cs/repeats/${props.item.id}/edit` },
]
</script>

<template>
  <Head title="Edit CS Repeat" />
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="mx-6 mt-6 space-y-6">
    <Card>
      <CardHeader>
        <CardTitle>Edit CS Repeat</CardTitle>
      </CardHeader>
