    ...
    <CardContent>
      ...
      <div v-if="loading" class="flex items-center justify-center py-12">
        <div class="flex items-center gap-3 text-gray-500 dark:text-gray-400">
          <div class="animate-spin rounded-full h-6 w-6 border-2 border-teal-500 border-t-transparent"></div>
...
const canvasId = computed(() => `monthly-leads-chart-${canvasKey.value}`);

const chartData = computed<ChartData<'bar'> | null>(() => {
  if (!props.data || props.data.length === 0) return null;

  const labels = props.data.map(item => item.label);
  const values = props.data.map(item => Number(item.leads || 0));
    ...
    datasets: [
      {
        label: 'Leads',
        data: values,
        backgroundColor: 'rgba(20, 184, 166, 0.2)', // Teal soft
        borderColor: '#14b8a6',
        borderWidth: 2,
        borderRadius: 6,
        barThickness: 'flex',
  ...
  plugins: {
    ...
    tooltip: {
      ...
      callbacks: {
        ...
        label: (context) => `Jumlah Leads: ${Number(context.parsed.y || 0).toLocaleString('id-ID')}`,
      },
    },
  },
  scales: {
