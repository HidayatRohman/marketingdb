    ...
    <CardContent>
      ...
      <div v-else-if="chartData && (chartData.labels.length > 0)" class="relative">
        ...
        <div class="mt-3 grid grid-cols-1 gap-3 sm:mt-4 sm:gap-4 lg:grid-cols-2">
          <!-- Province Legend -->
          <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-3 max-h-60 overflow-y-auto">
            <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-2 flex items-center gap-2 sticky top-0 bg-white dark:bg-gray-800 py-1">
          ...
          <div class="rounded-lg border border-gray-200 dark:border-gray-700 p-3">
            ...
            <div class="space-y-1.5 sm:space-y-2">
              ...
              >
                <span class="text-xs font-medium text-indigo-900 dark:text-indigo-100 truncate max-w-[150px]">
                  {{ peak.provinsi || 'Unknown' }}
                </span>
...
import { TrendingUp, BarChart3, RefreshCw, MapPin } from 'lucide-vue-next';
...
interface ProvinceAnalyticsData {
  provinsi: string;
  count: number;
...
const viewMode = ref<'bar' | 'doughnut'>('bar'); // Default to bar for provinces

// Canvas management to avoid reuse issues
const canvasKey = ref(0);
const canvasId = computed(() => `province-analytics-chart-${canvasKey.value}`);

const getColorForLabel = (label: string | undefined, index: number) => {
  // Use hashed colors based on index for variety
  const hues = [210, 260, 300, 180, 20, 45, 90, 120, 150, 200, 330, 60, 280, 10];
...
const chartData = computed(() => {
  ...
  const labels = props.data.map(item => item.provinsi || 'Unknown');
      ...
      datasets: [
        {
          ...
          data: counts,
          backgroundColor: backgroundColors.map(c => c + '80'), // Add transparency
...
const countsByLabel = computed<Record<string, number>>(() => {
  ...
  (props.data || []).forEach(item => {
    const key = item.provinsi || 'Unknown';
...
const topProvinces = computed(() => {
  if (!props.data) return [] as { provinsi: string; count: number }[];
  return [...props.data]
    .sort((a, b) => b.count - a.count)
    .slice(0, 5) // Show top 5
    .map(item => ({ provinsi: item.provinsi || 'Unknown', count: item.count }));
});
...
const subtitleText = computed(() => {
  ...
  return `Distribusi transaksi per provinsi dalam ${periodLabel}`;
});
  ...
  const options: any = {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          title: (context: any) => {
            return `Provinsi: ${context[0].label || 'Unknown'}`;
          },
          label: (context: any) => {
