...
import { Loader2, AlertTriangle, Palette } from 'lucide-vue-next';
...
// Sinkronisasi data saat kendala berubah
watch(
    () => props.kendala,
    (newKendala) => {
        if (newKendala) {
            form.nama = newKendala.nama || '';
            form.warna = newKendala.warna || '#6B7280';
        } else {
            form.reset();
            form.warna = '#6B7280';
        }
...
const submit = () => {
    ...
    } else if (props.mode === 'edit' && props.kendala?.id) {
        form.put(`/kendalas/${props.kendala.id}`, {
            onSuccess: () => {
                emit('success');
                emit('close');
            },
        ...
        <DialogContent class="sm:max-w-[525px]">
            <DialogHeader>
                <DialogTitle class="flex items-center gap-2 text-xl">
                    ...
                    <span v-if="mode === 'create'">Tambah Kendala</span>
                    <span v-else-if="mode === 'edit'">Edit Kendala</span>
                    <span v-else>Detail Kendala</span>
                </DialogTitle>
                <DialogDescription>
                    Kelola daftar kendala dengan nama dan warna untuk kategorisasi.
                </DialogDescription>
            </DialogHeader>
            ...
            <form @submit.prevent="submit" class="space-y-6">
                <!-- Nama -->
                <div class="space-y-2">
                    <Label for="nama">Nama Kendala</Label>
                    <Input id="nama" v-model="form.nama" :disabled="mode === 'view'" placeholder="Masukkan nama kendala"
                ...
                <DialogFooter v-if="mode !== 'view'" class="gap-3">
                    ...
                    <Button type="submit" :disabled="form.processing" class="gap-2">
                        ...
                        <span v-if="mode === 'create'">Tambah Kendala</span>
                        <span v-else>Perbarui Kendala</span>
                    </Button>
                </DialogFooter>
            </form>
        </DialogContent>
