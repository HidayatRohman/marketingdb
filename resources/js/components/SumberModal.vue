...
import { Loader2, Palette, Globe } from 'lucide-vue-next';
...
// Watch for sumber prop changes
watch(
    () => props.sumber,
    (newSumber) => {
        if (newSumber) {
            form.nama = newSumber.nama || '';
            form.warna = newSumber.warna || '#6B7280';
        } else {
            form.reset();
            form.warna = '#6B7280';
        }
...
const submit = () => {
    ...
    } else if (props.mode === 'edit' && props.sumber?.id) {
        form.put(`/sumbers/${props.sumber.id}`, {
            onSuccess: () => {
                emit('success');
                emit('close');
            },
        ...
        <DialogContent class="max-h-[90vh] overflow-y-auto sm:max-w-[500px]">
            <DialogHeader class="space-y-3">
                <DialogTitle class="flex items-center gap-3 text-xl">
                    ...
                    <span v-if="mode === 'create'">Tambah Sumber Baru</span>
                    <span v-else-if="mode === 'edit'">Edit Sumber</span>
                    <span v-else>Detail Sumber</span>
                </DialogTitle>
                <DialogDescription class="text-base">
                    <span v-if="mode === 'create'">Buat sumber baru untuk kategorisasi transaksi dengan warna yang menarik.</span>
                    <span v-else-if="mode === 'edit'">Perbarui informasi sumber yang sudah ada.</span>
                    <span v-else>Informasi lengkap tentang sumber ini.</span>
                </DialogDescription>
            </DialogHeader>
            ...
            <form @submit.prevent="submit" class="space-y-8">
                ...
                <!-- Preview -->
                <div class="space-y-3">
                    <Label class="text-sm font-medium">Preview Sumber:</Label>
                    <div class="rounded-lg border-2 border-dashed border-gray-200 bg-gray-50/50 p-6 dark:border-gray-700 dark:bg-gray-800/50">
                        <div class="flex justify-center">
                            ...
                            >
                                ...
                                {{ form.nama || 'Nama Sumber' }}
                            </div>
                        </div>
                        <p class="mt-2 text-center text-xs text-gray-500 dark:text-gray-400">Begini tampilan sumber di halaman transaksi</p>
                    </div>
                </div>
            </form>
            ...
            <DialogFooter v-if="mode !== 'view'" class="gap-3 pt-6">
                ...
                <Button @click="submit" :disabled="form.processing" class="gap-2 px-6">
                    ...
                    <span v-if="mode === 'create'">Tambah Sumber</span>
                    <span v-else>Perbarui Sumber</span>
                </Button>
            </DialogFooter>
        </DialogContent>
    </Dialog>
